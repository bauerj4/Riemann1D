...
int HLLC_FLUX_MINBEE(mesh_t &mesh, vector<vector<double> > &fluxes, context_t &RiemannContext, double &smax)
{
  ...
  */
  ...
  for(int k = 2; k < mesh.NCells - 1; k++)
      ...
      else
	{
      ...
      if(rk[k-2] == rk[k-2])
	{
	  phi3 = MINBEE(rk[k-2],fabs(ckr[k-2]));
	}
      else
	{
	  phi3 = 1.;
